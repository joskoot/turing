...
(define (make-TM
  ...
  (define register-names
    (cond
      ...
      ((and
       ...
       registers)
      (else
       (error 'make-TM "incorrect registers argument: ~s" registers))))
      ...
      (cond
        ...
        (else
         (when (and limit (> move-nr limit))
           (error name "About to exceed max nr of ~s moves~nregisters: ~s~ntape: ~s"
                  limit
         ...
         (define new-registers
           (cons new-state
                 (cons new-datum
                       (for/list
                           ((new-register (in-list (cddr (rule-registers rule))))
  ...
  (define-values (normal-rule-hash dummy-rule-hash)
    (for/fold ((normal-hash (hash)) (dummy-hash (hash)))
              ((rule (in-list rules)))
  ...
  (define (check-TM-arguments input register-values report limit)
    ...
    (cond
      ...
      (register-values
       (error name "incorrect register-values: ~s" register-values)))
...
(define
  ...
  (unless (list? rules)
    (raise-argument-error 'make-TM "list of rules" rules))
  (for ((rule (in-list rules)))
    (unless (rule? rule)
      (error 'make-TM "incorrect rule: ~s" rule)))
  (let ((duplicate (check-duplicates rules #:key car)))
    (when duplicate
     (error 'make-TM "duplicate rule selector: ~s" duplicate))))
