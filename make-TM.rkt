...
(define (make-TM
    ...
    (let ((select (car x)) (instr (cadr x)))
     (and (= (length select) 2) (= (length instr) 3)
      (let
       ...
       (and
        (state? old-state)
        (or (tape-element? old-symbol) (dummy? dummy))
        (and
         (= (length new-states) (length state-reg-names))
         (andmap new-state? new-states)
         (andmap new-datum? new-data)
         (member move '(L R N)))))))))
  (unless (state? init-state) (error name "incorrect initial state: ~s" init-state))
  (unless (and (list? final-states) (andmap state? final-states))
   (error name "incorrect list of final states: ~s" final-states))
  (when (or (equal? dummy blank) (equal? dummy empty) (equal? blank empty))
   (error name "dummy blank and empty must be distinct, given: ~s ~s ~s" dummy blank empty))
  (unless (list? rules) (raise-argument-error name "list of rules" rules))
  (for ((rule (in-list rules)) #:unless (rule? rule)) (error name "incorrect rule: ~s" rule))
  (unless
   (or
    (exact-positive-integer? state-regs)
    (and (andmap keyword? state-regs) (not (check-duplicates state-regs))))
   (error name "incorrect state-registers: ~s" state-regs))
  (unless
   (or
    (exact-positive-integer? data-regs)
    (and (andmap keyword? data-regs) (not (check-duplicates data-regs))))
   (error name "incorrect data-registers: ~s" data-regs)))
 ...
 (define
  ...
  (define sym-reg-hash
   (if data
    (make-hash (map cons data-reg-names data))
  ...
  (define (TM-proper)
   ...
   (cond
    ...
    (else
     ...
     (define new-syms (map get-sym* (instr-new-syms instr) data-reg-names))
     ...
     (set! sym-reg-hash (make-hash (map cons data-reg-names new-syms)))
     ...
     (case report
      ((#t long)
       (printf
        ...
        line))
      ((short)
       (printf "~s: ~s~n" move-counter tape)))
     (TM-proper))))
