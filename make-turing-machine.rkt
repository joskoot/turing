...
Procedure: (make-turing-machine starting-state final-states blank user-blank report? rules)
           -> turing-machine
...
The produced turing-machine removes blanks and user-blanks preceding the content
...
(define (make-turing-machine starting-state final-states report? blank user-blank rules)
 ...
 (define (tape-put tape symbol)
  (when (equal? symbol blank) (error 'turing-machine "blank ~s not allowed to be written." blank))
  (let ((head (tape-head tape)) (tail (tape-tail tape)))
   (cond
 ...
 (define (find-rule state symbol rules)
  (let ((entry (assoc state rules)))
   ...
   (let ((entry (assoc symbol (cdr entry))))
    (unless entry (error 'turing-machine "no rule for symbol ~s in state ~s" symbol state))
 ...
 (λ (lst)
  ...
  (when (member blank lst)
   (error 'turing-machine "blank ~s not allowed in input" blank))
  (when (member user-blank lst)
   (error 'turing-machine "user-blank ~s not allowed in input" user-blank))
